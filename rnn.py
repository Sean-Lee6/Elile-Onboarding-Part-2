...
def main():
    ...
    train_dataframe = pd.read_csv('../Datasets/hourly/weather_train.csv', sep=";", engine='python')
    valid_dataframe = pd.read_csv('../Datasets/hourly/weather_dev.csv', sep=";", engine='python')
    test_dataframe = pd.read_csv('../Datasets/hourly/weather_test.csv', sep=";", engine='python')
    train_data, train_labels, valid_data, valid_labels, test_data, test_labels, scale_factor = import_data(train_dataframe, valid_dataframe, test_dataframe)
    ...
    mse_list.append(('Vanilla LSTM Train MSE', train_mse))
    mse_list.append(('Vanilla LSTM Valid MSE', valid_mse))
    mse_list.append(('Vanilla LSTM Test MSE', test_mse))
    #Adaboost model (ensemble learning)

    #cv_model = KerasClassifier(build_fn=build_model, epochs=100, batch_size=16, verbose=0)
    
    
    #I was not able to get the adaboost model to properly work
    
    # adaboost_base_estimator = KerasRegressor(model = build_model, optimizer="adam", epochs=100, batch_size = 16, verbose=0)
    ...
    #The k-fold below was for selecting hyperparameters, but we've already which ones worked best.
    ...
    write_to_csv(validset_predicted,'nn_validset_prediction.csv')
    write_to_csv(testset_predicted, 'nn_testset_prediction.csv')
    store_MSE(mse_list, 'mse_list.txt')
    return
