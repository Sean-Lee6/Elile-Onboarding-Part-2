...
def import_data(train_dataframe, valid_dataframe, test_dataframe):
    ...
    #np.r_ allows you to select discontinuous columns such as 0:12 (so not including
    #column 12 which is the output column), then 13 to the rest. Pretty cool
    train_data = dataset[:,np.r_[0:12, 13:17]]
    train_labels = dataset[:,12]

    # Create valid dataset
    dataset = valid_dataframe.values
    dataset = dataset.astype('float32')
    #Remember to scale validation/test set using TRAINING scale
    for i in range(0, 17):
        dataset[:, i] = normalize_data(dataset[:, i], min[i], max[i])

    valid_data = dataset[:,np.r_[0:12, 13:17]]
    ...
    test_data = dataset[:, np.r_[0:12, 13:17]]
...
def main():
    ...
    data['hour_ago_humidity'] = data['relative_humidity'].shift(1)
    #This additional hour variable would help keep track of previous hour to handle when it's 6 am
    #am previous data was 17 o clock, 12 hours prior
    data['hour_ago_hour'] = data['hour'].shift(1)
    data['two_hours_ago_humidity'] = data['relative_humidity'].shift(2)
    data['two_hours_ago_hour'] = data['hour'].shift(2)
    data = data.iloc[2:]
    ...
    #For our dataset, lag didn't do much. This file is kept for
    #illustrative purposes to show this failure.
    #Note that I had experimented with other lag features as well, but
    #none of them helped decrease error
    ...
    mse_list.append(('LSTM (lag) Train MSE', train_mse))
    mse_list.append(('LSTM (lag) Valid MSE', valid_mse))
    mse_list.append(('LSTM (lag) Test MSE', test_mse))
    ...
    store_MSE(mse_list, 'mse_list_new_lag.txt')
    return
